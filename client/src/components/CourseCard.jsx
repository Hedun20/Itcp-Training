...
export function CourseCard({ course, progress }) {
  const percentage = progress?.percentage ?? (course.moduleCount && progress?.completedModuleIds
    ? Math.round((progress.completedModuleIds.length / course.moduleCount) * 100)
    : 0);
  return (
    <TrainingCard as="article" interactive className="course-card">
      ...
      <div className="course-card__body">
        <div className="course-card__meta"><span><Clock3 size={15} />{course.estimatedDuration || course.duration || 'Self-paced'}</span><span><Layers3 size={15} />{course.moduleCount ?? course.modules?.length ?? 0} modules</span></div>
