...
The client uses `BrowserRouter`. Configure the static host to serve `client/dist/index.html` as the fallback for application routes such as `/courses/...`, `/history`, `/auth/callback`, and `/admin/...`. Keep real asset files, `/api/v1`, and `/uploads` ahead of that fallback so they are never rewritten to HTML.
